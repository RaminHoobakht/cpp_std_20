...
int main() {
    ...
    for (size_t i{0UL}; i < 10; ++i) {
        ...
        } catch (mariam::SomethingIsWrong &ex_inner) {
            if (typeid(ex_inner) == typeid(mariam::Warning)) {
                std::cerr << typeid(ex_inner).name()
                          << "-inner catch block was caught: "
                          << ex_inner.what() << std::endl;
            } else {
                throw;
            }
        } catch (mariam::SomethingIsWrong &ex_outer) {
            std::cerr << typeid(ex_outer).name()
                      << " exception was caught: " << ex_outer.what()
                      << std::endl;
        }
    }

    std::cout << "\n #(00:58:56): The End ..." << std::endl;
    return EXIT_SUCCESS;
}
